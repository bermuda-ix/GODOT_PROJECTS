...
@onready var boss_countdown: RichTextLabel = $CanvasLayer/BossCountdown
...
func countdown_to_boss():
	...
	if _enemies_left>0 and _enemies_left!=null:
		...
		if not boss_area_on_screen.is_on_screen():
			if player.global_position.x-boss_area_on_screen.global_position.x>0:
				guide_arrow_right.visible=true
			else:
...
func _on_boss_arena_area_entered(area: Area2D) -> void:
	in_boss_area=true
	guide_arrow_right.visible=false
	guide_arrow_left.visible=false
