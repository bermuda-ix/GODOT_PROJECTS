...
func teleport(x_dir: float, y_dir: float, _global_position: Vector2) -> Vector2:
	print_debug(_global_position)
	print_debug("(", x_dir, ",", y_dir, ")")
	...
	#assert(teleport_dir_helper_rc.target_position==Vector2(x_dir, y_dir))
	...
	if teleport_dir_helper_rc.is_colliding():
		#assert(teleport_dir_helper_rc.target_position==Vector2(x_dir, y_dir))
		print_debug(teleport_dir_helper_rc.get_collider())
		print_debug("blocked!")
...
#func teleport_to_char(body: CharacterBody2D, _global_position: Vector2) -> Vector2:
	#teleport_dir_helper_rc.look_at(Vector2(body.global_position.x, body.global_position.y-100))
	#if teleport_dir_helper_rc.is_colliding():
		##assert(teleport_dir_helper_rc.target_position==Vector2(x_dir, y_dir))
		#print_debug(teleport_dir_helper_rc.get_collider())
		#print_debug("blocked!")
